...
try:
    ...
    _HAS_VLLM = True
except ImportError:  # vllm is optional; only needed for HarmBench
    LLM = None
    SamplingParams = None
...
def harmbench_judge_fn(prompts: List[str], responses: List[str]) -> List[int]:
    if not _HAS_VLLM:
        raise RuntimeError("HarmBench evaluation requires vllm; please install vllm to use this metric.")
